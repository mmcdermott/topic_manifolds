...
class Hypersurface():
    ...
    def segments_intersect(self, other):
        ...
        Returns
            ...
            <interiors_intersect>
        TODO(mmd): Need to return interiors intersect, endpoints intersect, segments are colinear.
        """
        ...
        # Parallel Checking
        # These are default values, but we need to pull them out to ensure we have consistent levels of
        # tolerance checking through the parallel and coincident detection steps.
        parallel_rtol = 1e-5
        parallel_atol = 1e-8
        is_parallel = np.isclose(np.abs(lines_cos_sim), 1, rtol=parallel_rtol, atol=parallel_atol)
        ...
        l1_st_dist_other  = l1_st_delta_other.norm()
        l1_end_dist_other = l1_end_delta_other.norm()
        l2_st_dist_self   = l2_st_delta_self.norm()
        l2_end_dist_self  = l2_end_delta_self.norm()

        # I'm not sure if this quite makes sense, but the motivation here is to use the possible flux in the
        # angle counted as still parallel to act as flux in the possiblity of colinearity/coincident
        # endpoints. This is an approximation. It may be wrong. The factor of 2 at the end is just for safety.
        max_norm = max(Vector(l1_end-l1_st).norm(), Vector(l2_end-l2_st).norm())
        coincident_tol = np.abs(
            np.sin(np.arccos(1-(parallel_atol + parallel_rtol))) * max_norm
        ) * 2

        l1_st_on_other  = np.isclose(l1_st_dist_other,  0, atol=coincident_tol)
        l1_end_on_other = np.isclose(l1_end_dist_other, 0, atol=coincident_tol)
        l2_st_on_self   = np.isclose(l2_st_dist_self,   0, atol=coincident_tol)
        l2_end_on_self  = np.isclose(l2_end_dist_self,  0, atol=coincident_tol)
        ...
        # Just adding the parallel exception specifically.
        is_collinear = (
            (l1_st_on_other and l1_end_on_other) or (l2_st_on_self and l2_end_on_self) or 
            (is_parallel and (l1_st_on_other or l1_end_on_other or l2_st_on_self or l2_end_on_self))
        )
        ...
        if is_collinear:
            ...
            assert not endpoints_coincident, (
                "Shouldn't be parallel but not collinear with only one coincident endpoint! Got\n"
                f"self = {self},\n"
                f"other = {other},\n"
                f"l1_st_proj_other = {l1_st_proj_other},\n"
                f"l1_end_proj_other = {l1_end_proj_other},\n"
                f"l2_st_proj_self = {l2_st_proj_self},\n"
                f"l2_end_proj_self = {l2_end_proj_self},\n"
                f"lines_cos_sim = {lines_cos_sim},\n"
...
class LabeledSimplicialManifold():
    def __init__(
        ...
        # TODO(mmd): Check and also set this if there are no 1D boundaries.
    ...
    def embed_in_RN(self, xs: Sequence):
        raise NotImplementedError("Can't embed yet!")

    def pairwise_embedded_distances(self, xs: Sequence, noise_sigma: float = 0):
        embedded_xs = self.embed_in_RN(xs)
        noise_vals  = np.random.randn(embedded_xs.shape) * noise_sigma

        return embedded_xs + noise_vals

    def efficient_pairwise_distances(self, xs: Sequence):
        ...
        for simplex_path, simplices_in_tiling in zip(shortest_simplex_paths, simplicial_tilings):
            ...
            for edge, boundary_simplex in boundary_edges.items():
                ...
                if is_parallel:
                    ...
                    # TODO(mmd): this is duplicate work!
    ...
    def embed_in_Rn(self, embedding_dim: int) -> np.ndarray:
        raise NotImplementedError
